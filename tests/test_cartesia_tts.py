...
def test_stream_speech_retries_opening_handshake_timeout(monkeypatch):
    ...
    monkeypatch.setattr(cartesia_tts.websockets, "connect", fake_connect)
    ...
    assert fake_websocket.sent[0]["transcript"] == "Hello."
    assert [message["type"] for message in messages] == ["chunk", "done"]
